  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (result['success'] == true) {
        ...
        print('      Match #${result['match_number']}, Round ${result['round_number']}');
        return true;
      } else {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (result['winner_new_elo'] != null) {
        ...
        print('      Winner: ${result['winner_old_elo']} → ${result['winner_new_elo']} (${result['winner_change'] >= 0 ? '+' : ''}${result['winner_change']})');
        print('      Loser: ${result['loser_old_elo']} → ${result['loser_new_elo']} (${result['loser_change'] >= 0 ? '+' : ''}${result['loser_change']})');
        return true;
      }
    }
    print('   ❌ FAILED: ${response.statusCode} - ${response.body}');
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (result['post_id'] != null) {
        print('   ✅ SUCCESS - Comment count updated to ${result['comment_count']}');
        return true;
      }
    }
    print('   ❌ FAILED: ${response.statusCode} - ${response.body}');
...
Future<void> _generateTestReport(Map<String, bool> results) async {
  ...
  print('📈 Overall Success Rate: $successRate% ($passed/$total functions passing)');
  print('\n🔍 Detailed Results:');
