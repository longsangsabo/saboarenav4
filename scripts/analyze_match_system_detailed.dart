...
Future<void> _analyzeMatchesTableStructure(String serviceKey, String baseUrl) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (matches.isNotEmpty) {
        ...
        print('   Γö£ΓöÇΓöÇ tournament_id: UUID (Tournament reference, nullable)');
        ...
        print('   Γö£ΓöÇΓöÇ match_type: VARCHAR (tournament/friendly/challenge/league)');
        print('   Γö£ΓöÇΓöÇ game_type: VARCHAR (8-ball/9-ball/straight-pool/etc.)');
        print('   Γö£ΓöÇΓöÇ format: VARCHAR (race-to-X/best-of-Y/time-limited)');
        ...
        print('   Γö£ΓöÇΓöÇ status: VARCHAR (pending/in_progress/completed/cancelled)');
        print('   ΓööΓöÇΓöÇ result_type: VARCHAR (normal/forfeit/timeout/disqualification)');
        
        print('\n   ≡ƒÅå TOURNAMENT CONTEXT:');
        print('   Γö£ΓöÇΓöÇ round_number: INTEGER (Tournament round)');
        print('   Γö£ΓöÇΓöÇ round_name: VARCHAR (Quarterfinal/Semifinal/Final/etc.)');
        ...
        print('   ΓööΓöÇΓöÇ currency_type: VARCHAR (USD/EUR/points/spa_points)');
...
Future<void> _analyzeMatchTypes(String serviceKey, String baseUrl) async {
  ...
  try {
    ...
    final response = await http.get(
      Uri.parse('$baseUrl/rest/v1/matches?select=match_type,game_type,format,max_games,is_elimination,stake_amount,prize_pool'),
      headers: {
        'Authorization': 'Bearer $serviceKey',
        'apikey': serviceKey,
      },
    ...
    if (response.statusCode == 200) {
      ...
      for (final match in matches) {
        ...
        matchTypeDistribution[matchType] = (matchTypeDistribution[matchType] ?? 0) + 1;
        
        // Count game types
        final gameType = matchData['game_type']?.toString() ?? 'unknown';
        gameTypeDistribution[gameType] = (gameTypeDistribution[gameType] ?? 0) + 1;
...
Future<void> _analyzeCurrentMatchData(String serviceKey, String baseUrl) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('   ΓööΓöÇΓöÇ Avg Duration: ${completedMatches > 0 ? (totalDuration / completedMatches).toStringAsFixed(1) : 0} minutes');
      
      print('\n   ≡ƒôê STATUS BREAKDOWN:');
      statusDistribution.forEach((status, count) {
        final percentage = ((count / matches.length) * 100).toStringAsFixed(1);
        final emoji = status == 'completed' ? 'Γ£à' :
                     status == 'in_progress' ? '≡ƒöä' :
                     status == 'pending' ? 'ΓÅ│' : 'Γ¥î';
...
Future<void> _analyzeMatchLifecycle(String serviceKey, String baseUrl) async {
  ...
  print('   Γöé  Γö£ΓöÇΓöÇ Match type determined (tournament/friendly/challenge)');
  print('   Γöé  Γö£ΓöÇΓöÇ Game settings configured (game_type, format, max_games)');
...
Future<void> _analyzeEloSystem(String serviceKey, String baseUrl) async {
  ...
  try {
    ...
    final response = await http.get(
      Uri.parse('$baseUrl/rest/v1/matches?select=player1_score,player2_score,match_type,status&status=eq.completed'),
      headers: {
        'Authorization': 'Bearer $serviceKey',
        'apikey': serviceKey,
      },
    ...
    if (response.statusCode == 200) {
      ...
      print('   Γö£ΓöÇΓöÇ Rating Calculation: Based on expected vs actual results');
      ...
      print('      New_Rating = Old_Rating + K * (Actual_Score - Expected_Score)');
      ...
      if (completedMatches.isNotEmpty) {
        ...
        print('   ΓööΓöÇΓöÇ Competitive Balance: ${((closematches / completedMatches.length) * 100).toStringAsFixed(1)}% close');
      }
...
Future<void> _analyzeMatchRelationships(String serviceKey, String baseUrl) async {
  print('≡ƒöù RELATIONSHIPS & DEPENDENCIES:');
  
  try {
    // Sample match for relationship analysis
    final response = await http.get(
      Uri.parse('$baseUrl/rest/v1/matches?select=*&tournament_id=not.is.null&limit=1'),
      headers: {
        'Authorization': 'Bearer $serviceKey',
        'apikey': serviceKey,
      },
    ...
    if (response.statusCode == 200) {
      ...
      print('   Γö£ΓöÇΓöÇ matches ΓåÆ users (player1_id, player2_id, winner_id)');
      ...
      if (matches.isNotEmpty) {
        ...
        print('      Γö£ΓöÇΓöÇ Tournament: ${match['tournament_id'] ?? 'None'}');
        print('      Γö£ΓöÇΓöÇ Players: ${match['player1_id']} vs ${match['player2_id']}');
