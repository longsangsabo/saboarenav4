...
Future<void> testHandicapCalculations() async {
  ...
  for (var testCase in testCases) {
    ...
    print('📋 Test: ${testCase['challenger']} vs ${testCase['opponent']} (${testCase['bet']} SPA)');
...
Map<String, dynamic> calculateSaboHandicap(
  ...
  final subRankDiff = opponentValue - challengerValue; // Positive = opponent stronger
  ...
  if (isValid) {
    ...
    if (absSubRankDiff == 0) {
      ...
      explanation = '${subRankDiff > 0 ? challengerRank : opponentRank} được cộng $handicapAmount bàn ban đầu (chênh 1 sub-rank)';
      ...
      explanation = '${subRankDiff > 0 ? challengerRank : opponentRank} được cộng $handicapAmount bàn ban đầu (chênh 1 main rank)';
    } else if (absSubRankDiff == 3) {
      // 1 main rank + 1 sub-rank
      final handicapAmount = config['handicap_1_rank'] + config['handicap_05_rank'];
      if (subRankDiff > 0) {
        handicapChallenger = handicapAmount; // Challenger weaker
      } else {
        handicapOpponent = handicapAmount; // Opponent weaker
      }
      explanation = '${subRankDiff > 0 ? challengerRank : opponentRank} được cộng $handicapAmount bàn ban đầu (chênh 1 main rank + 1 sub-rank)';
      ...
      explanation = '${subRankDiff > 0 ? challengerRank : opponentRank} được cộng $handicapAmount bàn ban đầu (chênh 2 main rank)';
    }
  }
