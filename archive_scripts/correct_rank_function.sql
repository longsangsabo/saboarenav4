-- CORRECT FIX: Using RANK_REQUESTS table instead of notifications
-- This is the correct table where rank requests are stored!
...
BEGIN
    ...
    -- Get pending requests from RANK_REQUESTS table (not notifications!)
    SELECT json_agg(
        json_build_object(
            'id', rr.id,
            'user_id', rr.user_id,
