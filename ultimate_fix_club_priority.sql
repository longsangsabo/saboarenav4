-- FINAL ULTIMATE FIX: Fix club priority logic 
-- Priority: Owned clubs first, then memberships
...
BEGIN
    ...
    IF NOT v_is_service_role THEN
        ...
        -- Get user's club - PRIORITY FIX: Check owned clubs FIRST
        IF NOT v_is_admin THEN
            -- First priority: clubs owned by user
            SELECT id INTO v_user_club_id FROM clubs WHERE owner_id = v_user_id LIMIT 1;
            
            -- Second priority: clubs where user is active member
